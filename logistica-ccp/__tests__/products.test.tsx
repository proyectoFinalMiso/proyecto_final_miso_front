...
describe('Vista de creación de productos', () => {
  ...
  it('Validar renderizado de la vista', async () => {
    fetchMock.mockResponseOnce(
      JSON.stringify([{ id: 1, name: 'Fabricante A' }])
    );
    render(<Products />);
    await waitFor(() => {
  ...
  it('Renderizar tabla principal', async () => {
    fetchMock.mockResponseOnce(
      JSON.stringify([
        {
          id: 1,
          nombreProducto: 'Lápiz CarbonGraph Caja x 12 und',
          sku: 10001,
          volumen: 0.001,
          fabricante: 'Comercializadora El Sol',
          valorUnitario: '$14200 COP',
          fechaCreacion: '2024-03-24 04:34:12',
        },
      ])
    );
    render(<Products />);
