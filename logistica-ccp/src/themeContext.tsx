...
export const ThemeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [font, setFont] = useState<'plusJakartaSans' | 'lexend'>('plusJakartaSans');
  ...
  useEffect(() => {
    ...
    const parsedFontSize = storedFontSizeRaw ? parseInt(storedFontSizeRaw, 10) : null;
  ...
  const theme = useMemo(() => getTheme(mode, font, fontSize), [mode, font, fontSize]);

  if (!hasMounted) return null;

  return (
    <ThemeContext.Provider value={{ toggleTheme, toggleFont, mode, font, fontSize, setFontSize }}>
