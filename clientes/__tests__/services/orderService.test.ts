...
import { sendOrder, fetchClientOrders, OrderPayload } from '../../services/api/orderService';
...
describe('orderService', () => {
    ...
    describe('fetchClientOrders', () => {
        it('should fetch orders for a client', async () => {
            const mockOrders = {
                pedidos: [
                    {
                        id: 'order1',
            ...
            const result = await fetchClientOrders('cliente1');

            expect(mockedAxios.get).toHaveBeenCalledWith(
                `${API_BASE_URL}/pedidos?cliente_id=cliente1`
            );
